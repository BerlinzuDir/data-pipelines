...
from dags.shop_390.sync import GOOGLE_SHEETS_ADDRESS, GOOGLE_DRIVE_ADDRESS, TRADER_ID, FTP_ENDPOINT
...
def _load_all_files_to_sftp(store_id: str, file_list: pd.DataFrame) -> pd.DataFrame:
    ...
    with _connect_to_sftp(credentials) as sftp_client:
        file_list["sftp_filename"].apply(lambda title: _load_single_image_to_sftp(sftp_client, store_id, title))
    return file_list
