...
@vcr.use_cassette(
    "dags/order_notifications/get_order_data/endpoint_wrappers/cassettes/authentication_token.yaml"
)
@vcr.use_cassette(
...
def test_drivers_overview_returns_json_with_stops():
    drivers_overview = R.pipe(
        get_auth_context, get_delivery_api_auth_token, get_tour_overview
...
def test_pickup_overview_returns_json_with_orders():
    token = R.pipe(get_auth_context, get_delivery_api_auth_token)(
        "/delivery_api_credentials.json"
    )
    ...
    assert (
        pickup["orders"][0]["fkUserId"]["email"] == "jakob.j.kolb@gmail.com"
    )  # email of first client
